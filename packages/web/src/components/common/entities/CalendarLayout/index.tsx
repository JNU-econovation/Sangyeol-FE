...
const CalendarLayout = memo(function Calendar({
  ...
  const monthPicker = useRef<typeof MonthPicker>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        monthPicker.current &&
        !(monthPicker.current as any).contains(event.target)
      ) {
  ...
  return (
    <div>
      <div className="w-full flex justify-center">
        <button onClick={handleMonthPickerOpen}>
      ...
      {openMonthPicker && (
        <MonthPicker
          ...
          onCancel={() => setOpenMonthPicker(false)}
        />
      )}
    </div>
  );
